...
class WalkingFlat(BenchmarkBase):
    ...
    def step(self, action):
        ...
        reward2 = max(2.0-np.linalg.norm(action),0.0)*0.001
...
class SoftBridge(BenchmarkBase):
    ...
    def step(self, action):
        ...
        com_2 = np.mean(pos_2, 1)
        reward = (com_2[0] - com_1[0])
        reward2 = max(0.1-abs(com_2[1] - com_1[1]),0.0)
        ...
        # observation, reward, has simulation met termination conditions, debugging info
        # multi-objective version: 1. raw reward 2. the position change in y-axis
        return obs, reward, done, {'obj':np.array([reward,reward2])}
...
class Duck(BenchmarkBase):
    ...
    def step(self, action):
        ...
        if self.oldPos == None:
            ...
            reward2 = 0.1 - abs(abs(com_2[0]-com_1[0])-abs(com_1[0] - self.oldPos))
