...
from morl.utils import CheckMOGYM, FormulateReward
...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, env_params=None):
    def _thunk():
        if env_id.startswith("dm"):
            ...
            env = utils.pbmorl_make_env(env_id)
            #env = gym.make(env_id)
            if env_params:
        ...
        if not utils.CheckMOGYM(env_id):
            env.seed(seed + rank)
...
class TimeLimitMask(gym.Wrapper):
    def step(self, action):
        packedReturn = self.env.step(action)
        obs,done,info = FormulateReward(packedReturn)
        rew=0.
